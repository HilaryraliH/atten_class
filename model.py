...
def Smaller_Resnet_DeepConvNet(model_input, Chans, nb_classes=2, dropoutRate=0.5):
    # article: EEGNet: a compact convolutional neural network for EEG-based brain–computer interfaces
    # changed as the comments
    block1 = Conv2D(8, (1, 5), input_shape=(Chans, Samples, 1), kernel_constraint=max_norm(2., axis=(0, 1, 2)))(
        model_input)  # it's channel first before
    ...
    block2 = Conv2D(16, (1, 5), padding='same',kernel_constraint=max_norm(
        2., axis=(0, 1, 2)))(block1)
    block2 = BatchNormalization(epsilon=1e-05, momentum=0.1)(block2)
    block2 = Activation('elu')(block2)

    block1 = Conv2D(16,(1,1),padding='same')(block1)
    block2 = Add()([block1,block2]) # 第一条resnet

    block2 = MaxPooling2D(pool_size=(1, 2), strides=(1, 2))(block2)
    block2 = Dropout(dropoutRate)(block2)

    block3 = Conv2D(32, (1, 5), padding='same',kernel_constraint=max_norm(
        2., axis=(0, 1, 2)))(block2)
    block3 = BatchNormalization(epsilon=1e-05, momentum=0.1)(block3)
    block3 = Activation('elu')(block3)

    block2 = Conv2D(32,(1,1),padding='same')(block2)
    block3 = Add()([block2,block3]) # 第二条连接

    block3 = MaxPooling2D(pool_size=(1, 2), strides=(1, 2))(block3)
    block3 = Dropout(dropoutRate)(block3)

    block4 = Conv2D(64, (1, 5), padding='same',kernel_constraint=max_norm(
        2., axis=(0, 1, 2)))(block3)
    block4 = BatchNormalization(epsilon=1e-05, momentum=0.1)(block4)
    block4 = Activation('elu')(block4)

    block3 = Conv2D(64,(1,1),padding='same')(block3)
    block4 = Add()([block3,block4]) # 第三条连接
...
def Smaller_Deeper_DeepConvNet(model_input, Chans, nb_classes=2, dropoutRate=0.5):
    # article: EEGNet: a compact convolutional neural network for EEG-based brain–computer interfaces
    # changed as the comments
    block1 = Conv2D(8, (1, 5), input_shape=(Chans, Samples, 1), kernel_constraint=max_norm(2., axis=(0, 1, 2)))(
        model_input)  # it's channel first before
...
def Smaller_Resnet_Deeper_DeepConvNet(model_input, Chans, nb_classes=2, dropoutRate=0.5):
    # article: EEGNet: a compact convolutional neural network for EEG-based brain–computer interfaces
    # changed as the comments
    block1 = Conv2D(8, (1, 5), input_shape=(Chans, Samples, 1), kernel_constraint=max_norm(2., axis=(0, 1, 2)))(
        model_input)  # it's channel first before
    ...
    block1 = Conv2D(16,(1,1),padding='same')(block1)
    block2 = Add()([block1,block2]) # 第一条连接
